...
const idx=JSON.parse(fs.readFileSync(path.join(root,'data/answers/answers-index.json'),'utf8')); if(!Array.isArray(idx)||!idx.length){ console.error('ANSWER COVERAGE FAILED: answers-index.json carries zero records. An empty loop is not coverage.'); process.exit(1); }
let bad=[];
